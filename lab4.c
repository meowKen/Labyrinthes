...
void AffLab(Tab t, int mF, int nF)
{
	...
	{	printf("\n");
		...
		{	
				...
				else if	(t[i][j] == 8)	printf("oo"); 
				else if	(t[i][j] == 1)	printf(". ");
				else if (t[i][j] == 4)  printf("00");
				else 					printf("  ");
		}
	}
}

void AffTabNumChemin(Tab t,int mF,int nF)	//debug - matrice de controle
{						//affiche "+" pour les murs et le numero du chemin de la case pour le reste			
...
void FaireChemin(Tab t, int mF, int nF, int finiF)	//creation d'un labyrinthe parfait (pas d'ilots - cad pas de cases inaxessible entourée de 4 murs)
{
...
void entreeSortie(Tab t, int m, int n)
{	//rand()%((MAX-MIN)+1)+MIN
	int xAlea = rand()%((m-1)+1)+1;

	while(t[xAlea][1]==-1)			//positionne l'entrée
	{
		xAlea = rand()%(((m-1)-1)+1)+1;
	}
	t[xAlea][0] = 8; 						//8 et 4 =entree/sortie du labyrinthes

	xAlea = rand()%((m-1)+1)+1;
	while(t[xAlea][n-2]==-1)		//positionne la sortie
	{
		xAlea = rand()%(((m-1)-1)+1)+1;
	}
	t[xAlea][n-2] = 4;
